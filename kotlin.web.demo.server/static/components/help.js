    ...
    if (helpForExamples.length == 0 && counter < 10) {
        ...
        while (helpForExamples[i] != undefined) {
            ...
            if (helpForExamples[i].fullMode == "") {
                $("div[id='bullet" + name + "']").css("background", "url(/static/icons/java.png) no-repeat");
                $("div[id='bullet" + name + "']").attr("title", "java");
            } else {
                $("div[id='bullet" + name + "']").css("background", "url(/static/icons/" + replaceAll(helpForExamples[i].fullMode, " ", "") + ".png) no-repeat");
