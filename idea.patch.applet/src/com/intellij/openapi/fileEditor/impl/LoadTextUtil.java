...
public final class LoadTextUtil {
  private static final Logger LOG = Logger.getInstance("#org.jetbrains.jet.internal.com.intellij.openapi.fileEditor.impl.LoadTextUtil");
  ...
  private static Pair<Charset,byte[]> getBOMAndCharset(@NotNull byte[] content, final Charset charset) {
    if (charset != null && charset.equals(CharsetToolkit.UTF8_CHARSET) && CharsetToolkit.hasUTF8Bom(content)) {
  ...
  private static void setDetectedFromBytesFlagBack(@NotNull VirtualFile virtualFile, @NotNull Charset charset, @NotNull String text) {
    ...
    CharsetToolkit.GuessedEncoding guessedEncoding = new CharsetToolkit(content, Charset.defaultCharset()).guessFromContent(content.length);
